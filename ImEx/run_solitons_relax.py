...
if __name__=="__main__":
    ...
    dt=0.01     ## Choose dt
    m = 2048
    tau_list = [0.01,0.001,0.0001,0.00001]#[1.0/np.power(10.0,i) for i in range(3,7)]
    ...
    x = np.arange(-m/2,m/2)*(L/m)
    xi = np.fft.fftfreq(m)*m*2*np.pi/L
   # print("Yo",xi.shape,L,40.0*np.pi,m)

    q0_ini  = sol
    q1_ini  = dx_sol
    u_ini = np.stack((q0_ini,q1_ini),axis=1)

    frm,tt,inv_change_dict,inv_fin,errs,mass_err_l,err_l = run_nls_example(dt,x,xi,kppa,T,imx,inv_list,u_ini,exact_soln_np,dx_soln_jx,log_errs=True,num_plots=1000)
    frame_dictNLS_list.append({"ini_type":ini_type,"scheme":imex_sch,"frame_list":frm,"t_list":tt,"inv_change_dict":inv_change_dict,"inv_final":inv_fin,"errors":errs,"x":x,"xi":xi,"kappa":kppa,"dt":dt,"m":m,"mass_err_l":mass_err_l,"err_l":err_l})
    #print("NLS inv_fin",inv_fin)

    frm,tt,inv_change_dict,inv_fin,errs,mass_err_l,err_l = run_nls_example(dt,x,xi,kppa,T,imx,inv_list,u_ini,exact_soln_np,dx_soln_jx,relax=True,log_errs=True,num_plots=1000)
    frame_dictNLS_relax_list.append({"ini_type":ini_type,"scheme":imex_sch,"frame_list":frm,"t_list":tt,"inv_change_dict":inv_change_dict,"inv_final":inv_fin,"errors":errs,"x":x,"xi":xi,"kappa":kppa,"dt":dt,"m":m,"mass_err_l":mass_err_l,"err_l":err_l})
    #print("NLS relaxn inv_fin",inv_fin)
    ...
    for tau in tau_list:
        ...
        frm,tt,inv_change_dict,inv_fin,errs,mass_err_l,err_l = run_nls_hyper_example(dt,x,xi,kppa,T,tau,lmda_list,imx,inv_list,u_ini,exact_soln_np,dx_soln_jx,log_errs=True,num_plots=1000)
        ...
        frm,tt,inv_change_dict,inv_fin,errs,mass_err_l,err_l = run_nls_hyper_example(dt,x,xi,kppa,T,tau,lmda_list,imx,inv_list,u_ini,exact_soln_np,dx_soln_jx,relax=True,log_errs=True,num_plots=1000)
        frame_dict_relax_list.append({"tau":tau,"ini_type":ini_type,"scheme":imex_sch,"frame_list":frm,"t_list":tt,"inv_change_dict":inv_change_dict,"inv_final":inv_fin,"errors":errs,"x":x,"xi":xi,"kappa":kppa,"dt":dt,"m":m,"mass_err_l":mass_err_l,"err_l":err_l})
        #print("relaxn inv_fin",inv_fin)
    ...
    file_name_relax = save_dir+"/"+case+"_tau_relax.pkl"
    file_NLSname = save_dir+"/"+case+"_tauNLS.pkl"
    file_NLSname_relax = save_dir+"/"+case+"_tau_relaxNLS.pkl"
    #file_name="tets.pkl"
    ...
    with open(file_name_relax, 'wb') as f:
        pickle.dump(frame_dict_relax_list,f)

    with open(file_NLSname, 'wb') as f:
        pickle.dump(frame_dictNLS_list,f)

    with open(file_NLSname_relax, 'wb') as f:
        pickle.dump(frame_dictNLS_relax_list,f)